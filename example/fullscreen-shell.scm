...
(define* (add-listen* obj symbol proc #:key (destroy-when         obj)
                                            (remove-when-destroy? #t))
  (let ([listener (make-wl-listener proc)])
    (wl-signal-add (get-event-signal obj symbol)
                   listener)
...
(define-class <fullscreen-output> ()
  ...
  (surface #:getter get-output-surface #:init-keyword #:surface #:setter set-output-surface))
...
(define (render-surface surface sx sy data)
  ...
  (let* ([rdata   (wrap-render-data data)]
    ...
    (when texture
      ...
      ;; };
      (let* ([box       (make-wlr-box (* sx                           (.scale output))
                                      (* sy                           (.scale output))
  ...
  (let-values ([(renderer)     (get-server-renderer (get-output-server fullscreen-output))]
    ...
    (when (wlr-output-attach-render (get-output-output fullscreen-output))
      ...
      (let* ([->16as10 (lambda (n) (inexact->exact (round (* 255 n))))]
             [color    (make-rgba-color (->16as10 0.3) (->16as10 0.3) (->16as10 0.3) (->16as10 1.0))])
        ;;; Line 99
      ...
      (when (get-output-surface fullscreen-output)
        ...
        ;; 	.when = &now,
        ;; };
        (let ([rdata (make <render-data> #:output   (get-output-output fullscreen-output)
                                         #:renderer renderer
                                         #:when     now)])
          ;;; Line 107
          ;; wlr_surface_for_each_surface(output->surface, render_surface, &rdata);
          (wlr-surface-for-each-surface (get-output-surface fullscreen-output) render-surface rdata))
...
(define ((server-handle-new-output server) listener data)
  ...
  (let* ([output            (wrap-wlr-output data)]
         [fullscreen-output (make <fullscreen-output> #:link    (make-wl-list)
                                                      ;;; Line 155
    ...
    (wlr-output-init-render output (get-server-allocator server) (get-server-renderer server))
    ...
    (wl-list-insert (get-server-outputs server) (get-output-link fullscreen-output))

    ;;; Line 160
    ;; wlr_output_layout_add_auto(server->output_layout, wlr_output);
    (wlr-output-layout-add-auto (get-server-output-layout server) (get-output-output fullscreen-output))
  ...
  (let ([event (wrap-wlr-fullscreen-shell-v1-present-surface-event data)])
    ...
    (wl-list-for-each (lambda (obj wl-lst)
                        (let ([out-put            (.output event)]
                          ...
                          (when (or (not out-put) (equal? out-put fullscreen-out-put))
                            ;;; Line 180
                            ;; output_set_surface(output, event->surface);
                            (output-set-surface fullscreen-out-put (.surface event)))))
